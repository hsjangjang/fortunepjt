...
class FortuneCalculator:
    ...
    def _build_fortune_prompt(
        ...
        if period_type == 'weekly':
            ...
            timing_guide = """
   ★★★ 주간 운세 필수 형식 (이 형식을 정확히 따르지 않으면 오류입니다) ★★★

   각 운세 항목은 반드시 아래 5문장 구조로 작성하세요:

   문장1: [총운] 이번 주 전체 흐름 요약 (예: "이번 주는 새로운 도전의 기운이 강합니다.")
   문장2: [평일 초] "평일 초(월~화)에는 ~합니다." (반드시 "평일 초(월~화)에는"으로 시작)
   문장3: [평일 말] "평일 말(수~금)에는 ~합니다." (반드시 "평일 말(수~금)에는"으로 시작)
   문장4: [주말] "주말(토~일)에는 ~합니다." (반드시 "주말(토~일)에는"으로 시작)
   문장5: [조언] 한 주 마무리 조언 (예: "꾸준히 노력하면 좋은 결과가 있을 것입니다.")

   ❌ 금지: "오전", "오후", "저녁", "점심", 구체적 시간, "오늘", "길일" 등 일일 표현
   ✅ 필수: 2~4번째 문장은 반드시 "평일 초(월~화)에는", "평일 말(수~금)에는", "주말(토~일)에는"으로 시작"""
            item_period = '이번 주'
            ...
            timing_guide = """
   ★★★ 월간 운세 필수 형식 (이 형식을 정확히 따르지 않으면 오류입니다) ★★★

   각 운세 항목은 반드시 아래 5문장 구조로 작성하세요:

   문장1: [총운] 이번 달 전체 흐름 요약 (예: "이번 달은 성장의 기운이 강합니다.")
   문장2: [월초] "월초(1~10일)에는 ~합니다." (반드시 "월초(1~10일)에는"으로 시작)
   문장3: [월중] "월중(11~20일)에는 ~합니다." (반드시 "월중(11~20일)에는"으로 시작)
   문장4: [월말] "월말(21일~)에는 ~합니다." (반드시 "월말(21일~)에는"으로 시작)
   문장5: [조언] 한 달 마무리 조언 (예: "차근차근 진행하면 좋은 결과를 얻습니다.")

   ❌ 금지: "오전", "오후", "저녁", "이번 주", "오늘", "길일" 등 일일/주간 표현
   ✅ 필수: 2~4번째 문장은 반드시 "월초(1~10일)에는", "월중(11~20일)에는", "월말(21일~)에는"으로 시작"""
            item_period = '이번 달'
        else:  # daily
            period_title = '오늘의 운세'
            period_word = '오늘'
